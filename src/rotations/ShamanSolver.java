...
public class ShamanSolver implements RotationSolver {
  ...
  // Buffs
  int mLightningShieldBuff[] = { 324, 325, 905, 945, 8134, 10431, 10432 };
  ...
  public ShamanSolver(BetterBot bot) {
    mBot = bot;
    mPlayer = bot.getPlayer();
    mKeyboard = bot.getKeyboard();
    mPlayerLevel = mPlayer.getLevel();

    System.out.println("TheCrux's Shaman script started");
  ...
  public void combat(Unit u) {
    ...
    if (mPlayer.getHealthFloat() > 0.60f) {
      ...
      if (targetDistance < 20) {
        ...
        if (targetDistance <= 5) {
          // Stormstrike
          if (mPlayerLevel >= 40 && targetDistance <= 5 && !mBot.anyOnCD(mStormstrike) && targetHealth > 0.1f && manaValue > 0.3f) {
            mKeyboard.type('5');
          }
          // Auto Attack
          else {
        ...
        if (mPlayerLevel >= 10 && targetHealth > 0.3f && manaValue > 0.15f) {
          // Flame Shock
          if (!u.hasAura(mFlameShock) && mPlayerLevel < 40 && !mBot.anyOnCD(mFlameShock) && !mBot.anyOnCD(mEarthShock)) {
            mKeyboard.type('2');
          }
          // Searing Totem
  ...
  boolean isFullBuffed() {
    ...
    // Lightning Shield
    if (mPlayerLevel >= 8 && !mPlayer.hasAura(mLightningShieldBuff) && mPlayer.getManaFloat() > 0.15f) {
      mKeyboard.type('6');
      return false;
    }
    // Strength of Earth Totem
    if (mPlayerLevel >= 10 && !mPlayer.hasAura(mStrengthOfEarthTotem) && mPlayer.getManaFloat() > 0.15f) {
      mKeyboard.type('8');
      return false;
    }
